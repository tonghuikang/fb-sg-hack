    ...
    b64(image_url, function(b64data) {
      ...
      xhr_ocr.onreadystatechange = function() {
        if (xhr_ocr.readyState === 4) {
          ...
          xhr_keyword_n.open("POST", 'https://language.googleapis.com/v1/documents:analyzeEntitySentiment?key=AIzaSyAu3-oR7sduccQjKHKlnoqFGT3L-ttneyA', true);
          ...
          xhr_keyword_n.onreadystatechange = function() {
            if (xhr_keyword_n.readyState === 4) {
              ...
              xhr_keyword_v.open("POST", 'https://language.googleapis.com/v1/documents:analyzeSyntax?key=AIzaSyAu3-oR7sduccQjKHKlnoqFGT3L-ttneyA', true);
              ...
              xhr_keyword_v.onreadystatechange = function() {
                if (xhr_keyword_v.readyState === 4) {
                  ...
                  var verbs_amount = root_verbs.length;
                  ...
                  //to make sure there aren't too many keywords
                  var nouns_limit = Math.floor((10 * nouns_amount)/(verbs_amount+nouns_amount));
                  var nouns_to_search = noun_list.slice(0,nouns_limit);
                  var nouns_string = nouns_to_search.join(' '); 
                  //to avoid error if there aren't any root verbs
                  if(verbs_amount != 0){
                    var verbs_limit = 10 - nouns_limit;
                    var verbs_to_search = root_verbs.slice(0,verbs_limit);
                    var verbs_string = verbs_to_search.join(' ');
                  }
